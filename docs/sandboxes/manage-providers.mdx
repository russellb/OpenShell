...
The proxy re-signs requests using SigV4 before forwarding to AWS. Clients that
send `Content-Length` bodies (curl, most CLI tools) work. Clients that use
`Transfer-Encoding: chunked` (e.g., boto3 `put_object`) are not yet supported
by the SigV4 relay and will fail.
